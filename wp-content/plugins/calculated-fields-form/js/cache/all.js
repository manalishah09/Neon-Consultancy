...
{var f=el.id.match(/_\d+$/),esc=$.fbuilder.escape_symbol,r;e=$.fbuilder['forms'][f[0]].getItem(el.name);r=new RegExp('^\\s*('+esc(e.currencySymbol)+')?\\s*\\-?\\d+('+esc(e.thousandSeparator)+'\\d{3})*('+e.centSeparator+'\\d+)?\\s*('+esc(e.currencyText)+')?\\s*$','i');return this.optional(el)||r.test(v)||$.isNumeric(v);},$.validator.messages['currency']);$.validator.methods.min=function(v,el,p)
